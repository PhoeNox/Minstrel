...
- ~~`Aspire/ServiceDefaults` remains referenced for the `just aspire` dev workflow; in the shipped binary its OpenTelemetry exporters no-op without a collector, so it is left in place.~~ Superseded: Aspire (AppHost + ServiceDefaults) was removed entirely — it modelled a dev-only topology (Vite dev servers) that the shipped single-process Backend never uses, and the Backend already self-hosts both frontends from `wwwroot`.
