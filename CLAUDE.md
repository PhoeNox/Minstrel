...
- **`Backend/`** — ASP.NET Core, in four layers: the pure `Core` modules, the stateful **`Sessions/`** coordinators, the I/O **providers** (`FileSystem`/`Network`), and the HTTP **`Api/`** endpoints. The coordinators wire the pure `Core` modules through **edge integrations** that carry no domain logic (ADR-0007):
  - **`Sessions/`** — the stateful coordinators, each holding live Core state behind one lock and broadcasting a per-state-change snapshot to its subscribers. The two differ in *what* they put on the wire (see below): playback renders the SSE frame once per broadcast, the timer emits raw Core records mapped per subscriber.
    - `PlaybackSession` holds `PlaybackState` + `PlaylistBook` — every command and every `SessionClock` tick takes its lock; it resolves a `SongId` via the Pool, drives a pure Playlist/Playback op, performs M3U I/O, reconciles the index-cursor from the structural change, and broadcasts a `SnapshotEvent`. Because the playback snapshot is identical for every subscriber, the session maps Core→wire and serializes the SSE frame *once* per broadcast — via the `PlaybackEndpoints.RenderFrame` delegate injected at construction — so `SnapshotEvent` carries an already-rendered frame string, not a raw Core record.
    - `TimerSession` is the independent countdown — own state, `SessionClock` tick, `/timer/sse` stream, and Gong one-shot — broadcasting `TimerEvent` as raw Core records: `TimerSnapshotEvent` carries the `TimerAnchor` and the endpoint maps Core→wire per subscriber.
  - **`Api/`** — thin HTTP endpoint adapters (`PlaybackEndpoints`, `PlaylistEndpoints`, `LibraryEndpoints`, `TimerEndpoints`, `SystemEndpoints`) over the sessions; also owns the wire DTOs and the pure `SnapshotMapper` (`SnapshotDtos.cs`, assembling `Playback ⨝ Playlist` and mapping Core→wire once per broadcast, since the snapshot is identical for every subscriber) plus the inline `TimerDto` and `GongOptions`.
